...
$app->post('/perfiles', function ($request,$response,array $args) {
    $params = (array)$request->getParsedBody();

    $id = $params['id'];
    $nombre = strtoupper($params['nombre']);

    $sql1="SELECT * FROM roles where UPPER(nombre) = UPPER('$nombre')";
    $db1=new DB();
    $conn1= $db1->connect();
...
$app->put('/perfiles/{id}', function ($request,$response,array $args) {
    $params = (array)$request->getParsedBody();
    ...
    $caso1=" SELECT * FROM roles WHERE UPPER(nombre) = UPPER('$nombre') and UPPER(nombre) =
    UPPER((SELECT nombre FROM roles WHERE id = '$id'))";
    $db1=new DB();
    $conn1= $db1->connect();
    ...
    //si es asi da error
    $caso2=" SELECT * FROM roles WHERE UPPER(nombre) = UPPER('$nombre') and UPPER(nombre) !=
    UPPER((SELECT nombre FROM roles WHERE id = '$id'))";
    $db2=new DB();
    $conn2= $db2->connect();
